...
public class DefaultPartitionCountTest {
  ...
  @Test
  void topicWithoutExplicitCountUsesDefault(KafkaContainer kafkaContainer) throws Exception {
  ...
  @Test
  void topicWithExplicitCountOverridesDefault(KafkaContainer kafkaContainer) throws Exception {
    try (AdminClient client = TestHelper.getAdminClient(kafkaContainer.getBootstrapServers())) {
  ...
  @Test
  void autoCreatedTopicUsesDefaultPartitionCount(KafkaContainer kafkaContainer) throws Exception {
    try (KafkaProducer<String, String> producer = TestHelper.buildProducer(kafkaContainer.getBootstrapServers())) {
