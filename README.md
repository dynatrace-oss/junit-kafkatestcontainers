...
| `envVars`           | *(none)*                         | Additional environment variables passed to the Kafka container. Applied after the library's own settings, so user-supplied values take precedence. Accepts `@KafkaTestcontainers.EnvVar(name, value)` entries. See [apache/kafka Docker image environment variables](https://hub.docker.com/r/apache/kafka#overriding-the-default-broker-configuration) for supported variables. |
